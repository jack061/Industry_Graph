    ...
    public class IOT_Order_SecRepository : RepositoryFactory<IOT_Order_SecEntity>, IIOT_Order_SecRepository
    {
        ...
        public void SaveChangeQForm(string keyValue, IOT_Order_SecEntity iOT_Order_SecEntity)
        {
            if (!string.IsNullOrEmpty(keyValue))
            {
                ...
                this.BaseRepository().ExecuteBySql(@" update IOT_Order_Sec set  QualifyQuantity='" + iOT_Order_SecEntity.QualifyQuantity + "' where Id='" + keyValue + "'");
            }
        ...
        {
            //获取正在进行中的订单Id
            var Id = this.BaseRepository().FindEntity(u => u.ProceedStatus == "1").Id;
