...
{	
	...
	/// IOT_Device_SecBLL
	/// </summary>	
	public class IOT_Device_SecBLL
	{
	    private IIOT_Device_SecRepository service=new IOT_Device_SecRepository();

		/// <summary>
        /// 缓存key
        /// </summary>
        public string cacheKey ="IOT_Device_SecCache" ;
        ...
        /// <returns></returns>
        public IEnumerable<IOT_Device_SecEntity> GetPageListBySql(Pagination pagination, string queryJson)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<IOT_Device_SecEntity> GetList( string queryJson)
        {
             var expression = LinqExtensions.True<IOT_Device_SecEntity>();
            var queryParam = queryJson.ToJObject();
            //查询条件
        ...
        /// <param name="queryJson">查询参数</param>
        /// <returns></returns>
        public IEnumerable<IOT_Device_SecEntity> GetPageList(Pagination pagination, string queryJson)
        {
			var expression = LinqExtensions.True<IOT_Device_SecEntity>();
            var queryParam = queryJson.ToJObject();
             //查询条件
            ...
            {
                string keyword = queryParam["keyword"].ToString();
                expression = expression.And(t => t.Name.Contains(keyword) ||t.Code.Equals(keyword));
            }
        ...
        public void SaveForm(string keyValue, IOT_Device_SecEntity iOT_Device_SecEntity)
        {
            try
            {
                service.SaveForm(keyValue, iOT_Device_SecEntity);
                CacheFactory.Cache().WriteCache(cacheKey,iOT_Device_SecEntity);
            }
            catch (Exception)
            {
                throw;
