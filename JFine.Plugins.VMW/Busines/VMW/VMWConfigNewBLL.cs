...
{	
	...
	/// VMWConfigNewBLL
	/// </summary>	
	public class VMWConfigNewBLL
	{
	    private IVMWConfigNewRepository service=new VMWConfigNewRepository();

		/// <summary>
        /// 缓存key
        /// </summary>
        public string cacheKey ="VMWConfigNewCache" ;
        ...
        /// <returns></returns>
        public IEnumerable<VMWConfigNewEntity> GetPageListBySql(Pagination pagination, string queryJson)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<VMWConfigNewEntity> GetList( string queryJson)
        {
             var expression = LinqExtensions.True<VMWConfigNewEntity>();
            var queryParam = queryJson.ToJObject();
            //查询条件
        ...
        /// <param name="queryJson">查询参数</param>
        /// <returns></returns>
        public IEnumerable<VMWConfigNewEntity> GetPageList(Pagination pagination, string bindId)
        {
			var expression = LinqExtensions.True<VMWConfigNewEntity>();
        ...
        public void SaveForm(string keyValue, VMWConfigNewEntity vMWConfigNewEntity)
        {
            try
            {
                service.SaveForm(keyValue, vMWConfigNewEntity);
                CacheFactory.Cache().WriteCache(cacheKey,vMWConfigNewEntity);
            }
            catch (Exception)
            {
                throw;
