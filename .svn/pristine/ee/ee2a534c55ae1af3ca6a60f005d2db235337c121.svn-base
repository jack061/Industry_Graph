...
{	
	...
	/// IOTBadRecordBLL
	/// </summary>	
	public class IOTBadRecordBLL
	{
	    private IIOTBadRecordRepository service=new IOTBadRecordRepository();

		/// <summary>
        /// 缓存key
        /// </summary>
        public string cacheKey ="IOTBadRecordCache" ;
        ...
        /// <returns></returns>
        public IEnumerable<IOTBadRecordEntity> GetPageListBySql(Pagination pagination, string queryJson)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<IOTBadRecordEntity> GetList( string queryJson)
        {
             var expression = LinqExtensions.True<IOTBadRecordEntity>();
            var queryParam = queryJson.ToJObject();
            //查询条件
        ...
        /// <param name="queryJson">查询参数</param>
        /// <returns></returns>
        public IEnumerable<IOTBadRecordEntity> GetPageList(Pagination pagination, string queryJson)
        {
			var expression = LinqExtensions.True<IOTBadRecordEntity>();
            var queryParam = queryJson.ToJObject();
             //查询条件
            if (!queryParam["keyword"].IsEmpty())
            {
                ...
                expression = expression.And(t => t.DeviceCode.Contains(keyord));
            }
        ...
        public void SaveForm(string keyValue, IOTBadRecordEntity iOTBadRecordEntity)
        {
            try
            {
                service.SaveForm(keyValue, iOTBadRecordEntity);
                CacheFactory.Cache().WriteCache(cacheKey,iOTBadRecordEntity);
            }
            catch (Exception)
            {
                throw;
