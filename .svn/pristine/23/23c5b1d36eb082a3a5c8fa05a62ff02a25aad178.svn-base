...
; !function (win) {
    ...
    Layer.prototype.view = function () {
        ...
        var title = (function () {
            ...
            return config.title
            ? '<h3 style="' + (titype ? config.title[1] : '') + '">' + ((config.topClose != undefined && config.topClose) ? '<div style="float:' + ((config.topClosePos != undefined && config.topClosePos == "left") ? 'left' : 'right') + ';" onclick="layer.closeAll();">关闭</div>' : '') + (titype ? config.title[0] : config.title) + '</h3>'
            : '';
        }());
