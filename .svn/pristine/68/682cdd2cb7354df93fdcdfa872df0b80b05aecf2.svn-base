            ...
            try
            {
                if (string.IsNullOrWhiteSpace(keyValue))
                {
                    ...
                    var list = GetListBySql(" AND BindId = '" + mQTTGatewayTagEntity.BindId
                        + "' AND ParameterCode = '" + mQTTGatewayTagEntity.ParameterCode + "' AND DeviceCode = '" + mQTTGatewayTagEntity.DeviceCode + "'");
                    if (list.Count() > 0)
                    {
                        throw new Exception("设备（" + mQTTGatewayTagEntity.DeviceCode + "）下已经存在参数编码（" + mQTTGatewayTagEntity.ParameterCode + "）；");
                    }
                }
                else 
                {
                    var list = GetListBySql(" AND BindId = '" + mQTTGatewayTagEntity.BindId
                        ...
                        + "' AND DeviceCode = '" + mQTTGatewayTagEntity.DeviceCode
                        + "' AND Id <> '" + keyValue + "'");
                    if (list.Count() > 0)
                    {
                        throw new Exception("设备（" + mQTTGatewayTagEntity.DeviceCode + "）下已经存在参数编码（" + mQTTGatewayTagEntity.ParameterCode + "）；");
