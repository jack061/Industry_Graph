...
{	
	...
	public class IOTDeviceRepository:RepositoryFactory<IOTDeviceEntity>, IIOTDeviceRepository
	{
        ...
        public DataTable LoadHourProductivity(string deviceCode)
        {
            DateTime dt = DateTime.Now;
            StringBuilder sql = new StringBuilder();
            sql.Append(@" select DeviceCode,Year,Month,DAY,Hour,count(1)as Quantity from dbo.IOT_DeviceBeat where Year =year(getdate()) and Month=month(getdate()) and day=day(getdate())  group by DeviceCode,Year,Month,DAY,Hour  having deviceCode='" + deviceCode + "' order by DeviceCode, Hour");
            return this.BaseRepository().FindTable(sql.ToString());
        }
        /// <summary>
        /// 实体
