...
GooFlow.prototype={
    ...
    //添加扩展信息
	addExpandInfo: function (id, name, type, data) {
	    if (type == "node") {
	        if (!this.$nodeData[id]) return;
	        var addNodeInfo = 'this.$nodeData[id].' + name + '=data';
	        eval(addNodeInfo);
	    } else if (type == "line")
	    {
	        if (!this.$lineData[id]) return;
	        var addLineInfo = 'this.$lineData[id].' + name + '=data';
	        eval(addLineInfo);
	    }
	},
	//设置结点的尺寸,仅支持非开始/结束结点
...
///////////以下为有关画线的方法
	...
	addLine:function(id,json){
		...
		this.$lineData[id].setting = json.setting;
