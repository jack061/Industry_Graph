...
(function ($) {
    ...
    function buildSelect(opts, $el) { // build Select options according to Select-to-Tree specification
        function buildOptions(dataArr, curLevel, pup) {
            for (var i = 0; i < dataArr.length; i++) {
                var data = dataArr[i] || {};
                var $opt = $("<option></option>");
                $opt.text(data[opts.treeData.labelFld || opts.text]);
                $opt.val(data[opts.treeData.valFld || opts.id]);
                if ($opt.val() === "") {
                ...
                $el.append($opt);
                var ChildNodes = data[opts.treeData.incFld || "ChildNodes"];
                if (ChildNodes) {
                    $opt.addClass("non-leaf");
                    buildOptions(ChildNodes, curLevel + 1, $opt.val());
                }
            }
        }
        buildOptions(opts.treeData.dataArr, 1, "");
        if (opts.treeData.dftVal) $el.val(opts.treeData.dftVal);
    }
