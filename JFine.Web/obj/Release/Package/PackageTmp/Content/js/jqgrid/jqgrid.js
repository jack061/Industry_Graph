...
(function (factory) {
    ...
    $.fn.jqGrid = function (pin) {
        ...
        return this.each(function () {
            ...
            formatCol = function (pos, rowInd, tv, rawObject, rowId, rdata) {
                ...
                var val = cm.vertical_align;
                if (val != undefined) { result += "vertical-align:" + val + ";"; }
            ...
            .click(function (e) {
                ...
                clickTimer = setTimeout(function () {
                    ...
                    ri = ptr[0].rowIndex;
                    ci = $.jgrid.getCellIndex(td);
                    var dbcr = $(ts).triggerHandler("jqGridClickRow", [$(ptr).attr("id"), ri, ci, e]);
                    if (dbcr != null) { return dbcr; }
                    if ($.isFunction(ts.p.onClickRow)) {
                        dbcr = ts.p.onClickRow.call(ts, $(ptr).attr("id"), ri, ci, e);
                        if (dbcr != null) { return dbcr; }
                    }
                }, 300);
            })
    ...
    $.jgrid.extend({
        ...
        getRowData: function (rowid, usedata) {
            ...
            this.each(function () {
                ...
                while (j < len) {
                    if (getall) { ind = $t.rows[j + 1]; }// 忽略第一条不可见的行
        ...
        getAllRows: function () {
            ...
            var rowNumber = o.jqGrid('getGridParam', 'rowNum'); //获取显示配置记录数量
            ...
            var totalRows = o.jqGrid('getRowData');  //输出所有匹配的
            //o.jqGrid('setGridParam', { rowNum: rowNumber }).trigger('reloadGrid'); //还原原来显示的记录数量
            return totalRows;
        },
        delRowData: function (rowid) {
            ...
            this.each(function () {
                ...
                if ($t.p.unwritten) {
                    $("#" + $t.p.id).parents('.ui-jqgrid-bdiv').find(".unwritten").remove();
                    if ($("#" + $t.p.id).find('tbody tr').length == 1) {
                        $("#" + $t.p.id).parents('.ui-jqgrid-bdiv').append('<div class="unwritten" style="color:red;padding:7px;border-bottom:#ddd 1px solid;">没有找到您要的相关数据!</div>')
                    }
                }
            });
            return success;
    ...
    $.extend($.jgrid, {
        ...
        checkValues: function (val, valref, customobject, nam) {
            ...
            if (edtrul) {
                ...
                if (edtrul.phone === true) {
                    if (!(rqfield === false && $.jgrid.isEmpty(val))) {
                        var length = val.length;
                        filter = /^(((13[0-9]{1})|(15[0-9]{1})|(18[0-9]{1}))+\d{8})$/;
                        if (!(length == 11 && filter.test(val))) { return [false, nm + ": " + msg.phone, ""]; }
                    }
                }
    ...
    $.jgrid.extend({
        ...
        endEditRows: function () {
            var success = true;
            var $t = this;
            var sss = $t.jqGrid('getDataIDs');
            $.each($t.jqGrid('getDataIDs'), function (index, ele) {
                success = success && $t.jqGrid('saveRow', ele, false, 'clientArray');
            });
            return success;
        },
